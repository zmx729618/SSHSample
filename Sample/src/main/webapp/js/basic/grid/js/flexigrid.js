/*var _myUserAgent = navigator.userAgent.toLowerCase();
 // Figure out what browser is being used
 jQuery.browser = {
 version: (_myUserAgent.match(/.+(?:rv|it|ra|ie)[\/: ]([\d.]+)/) || [])[1],
 safari: /webkit/.test(_myUserAgent),
 opera: /opera/.test(_myUserAgent),
 msie: /msie/.test(_myUserAgent) && !/opera/.test(_myUserAgent),
 mozilla: /mozilla/.test(_myUserAgent) && !/(compatible|webkit)/.test(_myUserAgent)
 };*/
/*
...
(function ($) {
    $.addFlex = function (t, p) {
        ...
        p = $.extend({ //apply default properties
            ...
            usepager: true,
            nowrap: true,
            use3rdparam: false,//process 函数是否启用第三个参数 第三个参数会将事先数据以json对象传过去
            page: 1, //current page
            total: 1, //total pages
            useRp: true, //use the results per page select box
            rp: 15, //results per page
            rpOptions: [10, 15, 20, 30], //allowed per-page values
            title: false,
            idProperty: 'id',
            pagestat: '显示第 {from} 条到 {to} 条,共 {total} 条记录',
            pagetext: 'Go:',//页数输入框前的文本
            outof: '/',
            ...
            checkbox: false,
            checkboxDisableProperty: '',
            disableValues: null,//单个值直接写值,多个值用放在[]中 用逗号分隔
            checkboxProperty: false,//复选框的属性,复选框以该属性作为name值,以该属性的值作为value的值;如果不设置则使用idProperty的值
            onChangePage: null
        }, p);
        ...
        var g = {
            ...
            addData: function (data) { //parse data
                ...
                $.each(data.rows, function (i, row) {
                    ...
                    var flag = 'data' + row[p.idProperty];
                    $.data(document.body, flag, row);
                    //添加复选框
                    if (p.checkbox) {
                        var cth = $('<th/>');
                        var checkDisable = '';
                        if (row[p.checkboxDisableProperty] && p.disableValues) {
                            var _dis = row[p.checkboxDisableProperty];
                            if ($.isArray(p.disableValues)) {
                                if ($.inArray(_dis, p.disableValues) > -1)
                                    checkDisable = ' disabled=true ';
                            } else if (typeof p.disableValues && !$.isArray(p.disableValues)) {
                                if (_dis === p.disableValues)
                                    checkDisable = ' disabled=true ';
                            }
                        }
                        var cthch;
                        if (p.checkboxProperty) {
                            cthch = $('<input type="checkbox" ' + checkDisable + ' name="' + p.checkboxProperty + '" value="' + row[p.checkboxProperty] + '"/>');
                        } else
                            cthch = $('<input type="checkbox" ' + checkDisable + ' name="' + p.idProperty + '" value="' + row[p.idProperty] + '"/>');
                        var objTr = $(tr);
                    ...
                    $('thead tr:first th', g.hDiv).each(//add cell
                        function () {
                            ...
                            if (typeof p.colModel[idx].align != 'undefined') {
                                td.align = p.colModel[idx].align;
                            } else {
                                td.align = this.align;
                            }
                            var _str = "row.";
                            if ((p.colModel[idx].name).indexOf(".") != -1) {
                                td.innerHTML = decodejson(p.colModel[idx].name, row);
                            }
                            else
...
//                        if (p.use3rdparam) {
                        var flag = 'data' + pid;
                        var row = $.data(document.body, flag);
                        if (pth.process) pth.process(tdDiv, pid, row);
                        /*  } else {
                ...
                $('tbody tr', g.bDiv).each(function () {
                    $(this).click(
                        function (e) {
                            ...
                            if (p.checkbox && ($(this).find('input')[0].disabled == true) && ($(this).hasClass('trSelected')))
                                $(this).removeClass('trSelected');
                            //添加多选框
                            if (p.checkbox) {
                                if ($(this).hasClass('trSelected')) {
                                    if ($(this).find('input')[0].disabled == false)
                        ...
                        function (e) {
                            if (e.shiftKey) {
                                ...
                                if (p.checkbox && ($(this).find('input')[0].disabled == true) && ($(this).hasClass('trSelected')))
                                    $(this).removeClass('trSelected');
                                if (p.checkbox && ($(this).find('input')[0].disabled == true))
                                    $(this).removeClass('trSelected');
                                if (p.checkbox) {
                                    if ($(this).hasClass('trSelected')) {
                                        $(this).find('input')[0].checked = true;
        ...
        if (p.colModel) { //create model if any
            ...
            for (var i = 0; i < p.colModel.length; i++) {
                ...
                if (cm) {    // only use cm if its defined
                    ...
                    if (cm.headerAligen) {
                        th.align = cm.headerAlign;
                    } else {
                        th.align = "center";
                    }
                    if (cm.align) {
                        th.cntentalign = cm.align;
                    }
                    if (cm.width) {
        ...
        if (p.usepager) {
            ...
            $(g.bDiv).after(g.pDiv);
            var html = ' <div class="pGroup"> <div class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">' + p.pagetext + ' <input type="text" size="4" value="1" style="top:-1px;"/> ' + p.outof + ' <span> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>';
            ...
            if (p.useRp) {
                ...
                $('.pDiv2', g.pDiv).prepend("<div class='pGroup'><select name='rp' style='margin-top:4px;'>" + opt + "</select></div> <div class='btnseparator'></div>");
