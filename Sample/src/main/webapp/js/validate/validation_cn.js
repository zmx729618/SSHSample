...
Validator.messageSource['en-us'] = [
	...
	['validate-numberNotZero' , 'Please enter a valid not zero number in this field.'],
	...
	['validate-onebit' , 'Please enter a valid number to retain up to one decimal.'],
	['validate-two' , 'Please enter a valid number to retain up to two decimal.'],	
	['validate-pattern' , 'Please enter a valid number to retain up to two decimal.'],
	['validate-three' , 'Please enter a valid number to retain up to three decimal.'],
	['validate-four' , 'Please enter a valid number to retain up to four decimal.'],
    //['validate-decimal' , 'Validation failed.'],
	...
	['equals-than','Please select FieldRecord.'],
	...
	['validate-oneCode','Please enter a valid number,one Numbers or capital letters,For example 1.'],
	['validate-threeCode','Please enter a valid number,three Numbers or capital letters,For example 100.'],
	['validate-fourCode','Please enter a valid number,four Numbers or capital letters,For example 1000.'],
	['validate-sixToEightCode','Please enter a valid number,between six and eight Numbers or capital letters,For example 100001.'],
	['validate-three-number' , 'Please ensure that the code is a three bit integer.'],
	['validate-five-number' , 'Please ensure that the code is a five bit integer.'],
	['validate-eight-number' , 'Please ensure that the code is a eight bit integer.'],
	['validate-fourDigitalOneCapital' , 'Please enter three digits and one digit-alpha(nonzero) and one capital!'],
	['validate-districtCode' , 'Please enter three digits and a digital or capital letters and one capital!'],//验证片区编码
	['validate-threeDigitalOneCapital', 'Please enter three digits and one digit-alpha(nonzero)!'],
	['validate-farmCode' , "Please enter three digits and a digital or capital letters!"],
	['validate-twoCapitalFourDigital' , "Please enter two capital  and four digits!"],
	['validate-plotCode' , 'Please enter one capital and three digits!'],//验证地块编码	
	['validate-signlessIntegral' , 'Please enter a valid signless integral!'],
	['validate-sixIntTwoDecimals','Please enter a maximum of six bit integer and a maximum of two bit decimals'],
	['max-district' , 'the value cannot be larger than the unassigned area of current section %s'], 
	['max-section' , 'the value cannot be larger than the unassigned area of current district %s'],
	['validate-threeDecimel' , 'Please enter a valid number to three decimal.']
];
...
Validator.messageSource['zh-cn'] = [
	...
	['required' , '此项不能为空,请输入值.'],
	['validate-number' , '请输入有效的数字.'],
	['validate-numberNotZero' , '请输入有效的非0数字.'],
	...
	//['validate-pattern' , '请输入最多保留小数点后两位的数字'],
	['validate-onebit' , '请您输入最多保留小数点后一位的正数.'],	// zhangqs
    ['validate-two' , '请您输入最多保留小数点后两位的正数.'],
	['validate-pattern' , '请您输入最多保留小数点后两位的正数.'],	// zhangqs
	['validate-three' , '请输入最多保留小数点后三位的正数.'],//zhangyf	
	['validate-four' , '请您输入最多保留小数点后四位的正数.'],	// zhangqs

	//['validate-decimal' , '请输入合法的批号'],
	...
	['validate-oneCode','请输入有效的编码,一位数字或大写字母,如:1.'],
	['validate-threeCode','请输入有效的编码,三位数字或大写字母,如:100.'],
	['validate-fourCode','请输入有效的编码,四位数字或大写字母,如:1000.'],
	['validate-sixToEightCode','请输入有效的编码,6~8位数字或大写字母,如:100001.'],
	['validate-three-number' , '请保证编码为三位整数.'],
	['validate-five-number' , '请保证编码为五位整数.'],
	['validate-eight-number' , '请保证编码为八位整数.'],
	['validate-fourDigitalOneCapital' , '请输入3位数字+1位非0数字或大写字母+1位大写字母!'],
	['validate-districtCode' , '请输入3位数字+1位数字或大写字母+1位大写字母!'],//验证片区编码
	['validate-threeDigitalOneCapital' , "请输入3位数字+1位非0数字或大写字母!"],//验证农场编码
	['validate-farmCode' , "请输入3位数字+1位数字或大写字母"],
	['validate-twoCapitalFourDigital' , "请输入2位大写字母+4位数字!"],
	['validate-plotCode' , '请输入1位大写字母+3位数字!'],//验证地块编码
	['validate-signlessIntegral' , '请输入有效的正整数!'],
	['validate-sixIntTwoDecimals' , '请输入最多6位整数+最多2位小数!'],
	['max-district' , '不能大于所属片区未分配面积%s'],
	['max-section' , '不能大于所属地块未分配面积%s'],
	['validate-threeDecimel' , '请输入最多保留小数点后三位小数']  //zhangwc
];
...
Validation.addAllThese([
	...
	/*
	// 一位小数的检验 zhangqs
	['validate-onebit',function(v,elm,args,metadata) {
		return eval('('+args.singleArgument+'.test(v))');
	}],
	// 四位小数的校验 zhangqs
	['validate-four',function(v,elm,args,metadata) {
		return eval('('+args.singleArgument+'.test(v))');
	}],
	
	['validate-editPrice',function(v,elm,args,metadata) {
		return eval('('+args.singleArgument+'.test(v))');
	}],	
	
	['validate-decimal',function(v,elm,args,metadata) {
		return eval('('+args.singleArgument+'.test(v))');
	}],
	*/
	/*
	 ...
	 * Usage : equals-than-值
	 */
	['equals-than', function(v,elm,args,metadata) {
				return v != args[0];
	}],
	/*
	 * Usage: max-district-值
	 */
	['max-district',function(v,elm,args,metadata) {
		return parseFloat(v) <= parseFloat(args[0]);
	},{depends : ['validate-number']}],
	/*
	 * Usage:Usage: max-section-值
	 */
	['max-section',function(v,elm,args,metadata) {
		return parseFloat(v) <= parseFloat(args[0]);
	},{depends : ['validate-number']}],
	...
	['validate-oneCode',/^[1-9A-Z]$/],
	['validate-threeCode',/^[1-9A-Z][\dA-Z]{2}$/],
	['validate-fourCode',/^[1-9A-Z][\dA-Z]{3}$/],
	['validate-sixToEightCode',/^[1-9A-Z][\dA-Z]{5,7}$/],
	['validate-onebit',/^[0-9]+(\.\d{1})?$/],
	['validate-pattern',/^[0-9]+((\.[0-9]{2})|(\.[0-9]{1}))?$/],
	['validate-three',/^[0-9]+((\.[0-9]{3})|(\.[0-9]{2})|(\.[0-9]{1}))?$/],
	['validate-four',/^[0-9]+(\.\d{1,4})?$/],	
	['validate-three-number',/^[1-9]\d{2}$/],
	['validate-five-number',/^[1-9]\d{4}$/],
	['validate-eight-number',/^[1-9]\d{7}$/],
	['validate-fourDigitalOneCapital',/^\d{3}[1-9A-Z]{1}[A-Z]{1}$/], 
	['validate-districtCode',/^\d{3}[0-9A-Z]{1}[A-Z]{1}$/], //片区
	['validate-threeDigitalOneCapital',/^\d{3}[1-9A-Z]{1}$/],
	['validate-farmCode',/^\d{3}[0-9A-Z]{1}$/],//农场
	['validate-two',/^[1-9]\d*(\.\d{1,2})?$|0\.+[1-9]{1}$|0$|0\.+[1-9]{1}[0-9]{0,1}$|0\.+[0-9]{1}[1-9]{1}$/],//输入保留两位小数的正数（不包括0）
	['validate-twoCapitalFourDigital' ,/^[A-Z]{2}[0-9]{4}$/],
	['validate-plotCode',/^[A-Z]{1}[0-9]{3}$/],
	['validate-signlessIntegral',/^[1-9]*[1-9][0-9]*$/],
	['validate-sixIntTwoDecimals',/^[0-9]{1,6}$|^\d{1,5}[.]\d{1,2}$/],
	['validate-threeDecimel',/^\-?[0-9]+((\.[0-9]{3})|(\.[0-9]{2})|(\.[0-9]{1}))?$/]  //zhangwc
]);
