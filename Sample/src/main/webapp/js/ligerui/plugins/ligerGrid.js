...
(function ($)
{
    ...
    $.ligerDefaults.GridString = {
        errorMessage: '发生错误',
        pageStatMessage: '显示第 {from} 条到 {to} 条，共 {total} 条数据',
    ...
    $.ligerDefaults.Grid.editors['select'] =
     ...
     //自定义编辑器
    $.ligerDefaults.Grid.editors['codeSelect'] =
     {
         create: function (container, editParm)
         {
             var column = editParm.column;
             //customData:自定义字段，可以扩展下拉框的data
             var columnData;
             //dataType
             if(column.editor.dataType=="0"){//母本
                columnData = editParm.record.FCodes;
             }else if(column.editor.dataType=="1"){//父本
                columnData = editParm.record.MCodes;
             }
             var input = $("<input type='text'/>");
             container.append(input);
             var options = {
                 ...
                 onSelected:function (value,text)
                 {
                   if(value != "" && text != ""){
                      if(column.editor.dataType=="0"){//母本
                          editParm.record.femaleId = value;
                          editParm.record.femaleCode = text;
                      }else if(column.editor.dataType=="1"){//父本
                          editParm.record.maleId = value;
                          editParm.record.maleCode = text;
                      }
                   }
                 },
                 onBeforeOpen:function(){
                       if(column.editor.dataType=="0"){//母本
                          if(editParm.record.FCodes==""){
                             return false;
                          }
                      }else if(column.editor.dataType=="1"){//父本
                          if(editParm.record.MCodes == ""){
                             return false;
                          }
                      }
                 }
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("                </div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'><span class='pcontrol'> <input type='text' size='6' value='1' style='width:20px' maxlength='4' /> / <span></span></span></div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
